...
def create_splats_with_optimizers(
    ...
    if init_type == "sfm" or init_type == "SfM" or init_type == "DA3" or init_type == "DA-3":
        ...
        with self.get_profiler(self.writer) as prof:
            ...
            for step in pbar:
                ...
                for batch_ind in range(self.cfg.batch_size):
                    ...
                    #print(f"\n=== AFTER COMPRESSION SIMULATION {step} ===")
                    #print("-- params --")
                    #for k, v in self.splats.items():
                    #    tensor_fingerprint(v.data, k)
                    #print("-- state tensors --")
                    ...
                    alpha = 2
                    offset_len = torch.mean(torch.norm(self.splats["offsets"],dim=2), dim=1) #(N,5,3)->(N,5)->(N)
                    K_knn = 4
                    t1=time.time()
                    if ((step % self.cfg.strategy.refine_every == 1) or (step == init_step) # step % self.cfg.strategy.refine_every == 0 then the pc is densified, knn has to be re-run the step after
                    ):
                        distance_matrix = knn(self.splats["anchors"].detach(), K=K_knn)
                        assert(distance_matrix[:,0].sum() ==0) #the distances of the point with itself == 0
                        radii = torch.mean(distance_matrix[:,1:],axis=1)
                        assert radii.shape[0] == offset_len.shape[0] #N
                        while (radii==0).any():
                            print("huston we're got a problem", K_knn)
                            K_knn +=1 
                            distance_matrix = knn(self.splats["anchors"].detach(), K=K_knn)
                            radii = torch.mean(distance_matrix[:,1:],axis=1)
                            assert K_knn < 10 #if all 10 nearest points are identical there is a problem in the code 
                        t2=time.time()
                    offset_radii_ratio = torch.mean((offset_len)/(radii))
                    loss_offset = torch.mean((torch.relu(offset_len-radii)/(radii))**alpha)
                    lambda_offset = 0
                    print("offset/radi ratio ", offset_radii_ratio)
                    loss = l1loss * (1.0 - cfg.ssim_lambda) + ssimloss * cfg.ssim_lambda + lambda_offset*loss_offset+ info["scales"].prod(dim=1).mean() * cfg.scale_reg + info["reg_loss"] * cfg.factor_reg + info["smooth_loss"] * cfg.smooth_reg
