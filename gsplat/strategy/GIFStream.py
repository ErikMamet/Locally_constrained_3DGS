...
#
    ...
    def _grow_gs(
        ...
        #def tensor_fingerprint(t: torch.Tensor, name: str):
        #    if t.numel() == 0:
        #        print(f"  {name}: EMPTY {tuple(t.shape)} {t.dtype}")
        #        return
        #    t_f = t.float()
        #    print(f"  {name}: shape={tuple(t.shape)} dtype={t.dtype} "
        #        f"sum={t_f.sum().item():.6f} mean={t_f.mean().item():.6f} "
        #        f"min={t_f.min().item():.6f} max={t_f.max().item():.6f} "
        #        f"norm={t_f.norm().item():.6f}")
        #print(f"\n=== _grow_gs called at step={step} ===")
        #print("-- params --")
        #for k, v in params.items():
        #    tensor_fingerprint(v.data, k)
        #print("-- state tensors --")
        #for k, v in state.items():
        #    if isinstance(v, torch.Tensor):
        #        tensor_fingerprint(v, k)
        #    else:
        #        print(f"  {k}: {v}")
        #print("=" * 60)
        ## ── END DIAGNOSTICS ─────────────────────────────────────────────────
        ...
        new_anchors_count = 0
        init_length = state["anchor_demon"].shape[0] * state["n_offsets"]
        for depth in range(self.update_depth):  # FIX: renamed loop var to avoid shadowing
            # update threshold
            cur_threshold = self.densify_grad_threshold * ((self.update_hierachy_factor//2)**depth)
            # mask from grad threshold
            candidate_mask = (grads_norm >= cur_threshold)
            candidate_mask = torch.logical_and(candidate_mask, offset_mask)
            ...
            size_factor = self.update_init_factor // (self.update_hierachy_factor**depth)
            cur_size = state["voxel_size"]*size_factor
            ...
            # sorted=True ensures deterministic ordering and inverse_indices
            selected_grid_coords_unique, inverse_indices = torch.unique(
                selected_grid_coords, return_inverse=True, dim=0, sorted=True
            )
            ...
            max_iters = grid_coords.shape[0] // chunk_size + (1 if grid_coords.shape[0] % chunk_size != 0 else 0)
            remove_duplicates_list = []
            for chunk_idx in range(max_iters):  # FIX: renamed to chunk_idx, no longer shadows depth
            ...
            if candidate_anchor.shape[0] > 0:
                new_scaling = torch.ones_like(candidate_anchor).repeat([1,2]).float().to(device)*cur_size
                new_scaling = torch.log(new_scaling)
                new_rotation = torch.zeros([candidate_anchor.shape[0], 4], device=device).float()
                new_rotation[:,0] = 1.0
                ...
                # FIX: replace scatter_max (non-deterministic on GPU) with 
                # scatter_reduce using 'amax' and deterministic grouping via sorting
                n_unique = selected_grid_coords_unique.shape[0]
                feat_dim = new_feat.size(1)
                time_dim = new_time_features.size(1)
                factors_dim = new_factors.size(1)

                # sort by inverse_indices for deterministic grouping
                sorted_order = torch.argsort(inverse_indices, stable=True)
                sorted_inv = inverse_indices[sorted_order]
                    
                def deterministic_scatter_max(src: torch.Tensor, idx: torch.Tensor, n: int) -> torch.Tensor:
                    # Works for src of any shape [N, *D]
                    leading = src.shape[0]
                    rest = src.shape[1:]  # could be (), (D,), (T, D), etc.
                    src_flat = src.reshape(leading, -1)  # [N, prod(rest)] or [N, 1] if scalar
                    if src_flat.shape[1] == 0:
                        src_flat = src_flat.unsqueeze(1)
                    D = src_flat.size(1)

                    out = torch.full((n, D), float('-inf'), device=src.device, dtype=src.dtype)
                    counts = torch.bincount(idx, minlength=n)
                    starts = torch.cat([torch.zeros(1, dtype=torch.long, device=idx.device), counts.cumsum(0)[:-1]])
                    for k in range(n):
                        s, c = starts[k].item(), counts[k].item()
                        if c > 0:
                            out[k] = src_flat[s:s+c].max(dim=0).values

                    return out.reshape(n, *rest) if rest else out.squeeze(1)

                new_feat = deterministic_scatter_max(new_feat[sorted_order], sorted_inv, n_unique)[remove_duplicates]
                new_time_features = deterministic_scatter_max(new_time_features[sorted_order], sorted_inv, n_unique)[remove_duplicates]
                new_factors = deterministic_scatter_max(new_factors[sorted_order], sorted_inv, n_unique)[remove_duplicates]
        ...
        prune_mask = (opacity_accum.sum(dim=-1,keepdim=True) < self.prune_opa*state["anchor_demon"].sum(dim=-1,keepdim=True)).squeeze(dim=1)
        anchor_demon = peak_ratio * state["anchor_demon"].max(dim=-1, keepdim=True)[0].repeat(1,state["anchor_demon"].shape[-1]) + (1-peak_ratio) * state["anchor_demon"].mean(dim=-1,keepdim=True)
        anchors_mask = (anchor_demon.sum(dim=-1,keepdim=True) > self.check_interval*self.success_threshold).squeeze(dim=1)
